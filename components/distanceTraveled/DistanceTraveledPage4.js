...
export default function DistanceTraveledPage4({ pastMonthDist }) {
  const pastMonthDistString = `${pastMonthDist != null ? pastMonthDist.toFixed(1) : "--"}m`;
  ...
  return (
    <View style={{ flex: 1, width: "100%" }}>
      ...
      <View style={DistanceTraveledStyles.center_container}>
        <Text style={DistanceTraveledStyles.muted}>PAST MONTH</Text>
        <Text style={[DistanceTraveledStyles.text, { fontSize: getDistFontSize(pastMonthDist) }]}>{pastMonthDistString}</Text>
      </View>
    </View>
  );
}
