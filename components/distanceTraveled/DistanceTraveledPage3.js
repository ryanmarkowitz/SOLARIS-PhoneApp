...
export default function DistanceTraveledPage3({ pastWeekDist }) {
  const pastWeekDistString = `${pastWeekDist != null ? pastWeekDist.toFixed(1) : "--"}m`;
  ...
  return (
    <View style={{ flex: 1, width: "100%" }}>
      ...
      <View style={DistanceTraveledStyles.center_container}>
        <Text style={DistanceTraveledStyles.muted}>PAST WEEK</Text>
        <Text style={[DistanceTraveledStyles.text, { fontSize: getDistFontSize(pastWeekDist) }]}>{pastWeekDistString}</Text>
      </View>
    </View>
  );
}
